$(function() {
	...
	function extFiltros() {
		...
		visent.filtros.resumo = formatarResumo;
	}
	...
	function criarComponente(interfaceFiltro, valores) {
		switch (interfaceFiltro.componente) {
			...
			default:
				visent.log('sem mapeamento: ', interfaceFiltro.componente, interfaceFiltro);
	...
	function CAMPO_CHECKBOX(interfaceFiltro) {
		return '<input type="checkbox" data-tipo="'+TIPO_CHK+'" data-idfiltro="'+interfaceFiltro.filtro.id+'" data-idinterfacefiltro="'+interfaceFiltro.id+'"/>';
	}
	...
	function carregarFiltros($tab, valoresFiltros, valoresTreeSalvos) {
		$tab.find('.resumo-filtros-sel').loading(true);
		...
		formatarResumo($tab);
		visent.configuracao.validarExecucao($tab);
		$tab.find('.resumo-filtros-sel').loading(false);
	}
...
//					filtro: {id: idfiltro},
	...
	function criarTree($tree, json, $actionBar, selected) {
		...
		if (nodes.length > 0) {
			var parent = $tree.treeview('getParent', nodes[0]);
	...
	function getActionBarHTML() {
		return '<div><form class="form-inline filtros-tree-action-bar">'
			...
			+'<span class="filtros-tree-action-bar-novo">'
				...
				+'<div class="btn-group btn-group-xs filtros-tree-action-bar-tipo">'
			...
			+'<span class="filtros-tree-action-bar-edit">'
				+'<div class="btn-group btn-group-xs">'
					...
					+'<button title="Copiar" class="btn btn-xs filtros-tree-action-bar-copy"><i class="glyphicon glyphicon-copy"></i></button>'
					+'<button title="Colar" class="btn btn-xs filtros-tree-action-bar-paste"><i class="glyphicon glyphicon-paste"></i></button>'
				+'</div>'
			+'</span>'
			+'<div class="btn-group btn-group-xs pull-right">'
				+'<button title="Download" data-tipo="down" class="btn btn-xs filtros-tree-action-bar-ext"><i class="glyphicon glyphicon-download"></i></button>'
				+'<button title="Upload" data-tipo="up" class="btn btn-xs filtros-tree-action-bar-ext"><i class="glyphicon glyphicon-upload"></i></button>'
			+'</div>'
			+'</form>'
	...
	function bindActionBar($dlg, $tree, $actionBar, filtroId, selected) {
		...
		$actionBar.find('.filtros-tree-action-bar-tipo button').on('click', function(e) {
			e.preventDefault();
		...
		// COPIAR
		$actionBar.find('.filtros-tree-action-bar-copy').on('click', function(e) {
			e.preventDefault();
			var id = $actionBar.find('.tree-nome').data('id');
			$actionBar.data('copyID', id);
			visent.log(id+' copiado');
		});
		
		// COLAR
		$actionBar.find('.filtros-tree-action-bar-paste').on('click', function(e) {
			e.preventDefault();
			var targetID = $actionBar.find('.tree-nome').data('id');
			var copyID = $actionBar.data('copyID');
			visent.log('Copiando '+copyID+' para '+targetID);
			$dlg.loading(true);
			visent.ws({
				path: 'filtros-tree/copy/'+copyID+'/'+targetID,
				method: 'POST',
				always: function() {
					$dlg.loading(false);
					reRender();
	...
	function formatarResumo($tab) {
		...
		var $div = $tab.find('.resumo-filtros-sel-content');
		$div.empty();
