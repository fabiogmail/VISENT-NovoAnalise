...
//    		scrollBar: true,
    ...
    function bindSections() {
    	$('.btn-rel-agend .btn-rel').on('click', function(e) {
    		e.preventDefault();
    		$.fn.fullpage.moveSectionUp();
    	});
    	$('.btn-rel-agend .btn-agend').on('click', function(e) {
    		e.preventDefault();
    		var idRel = null;
    		if ($('#section-rel .slide.active').attr('id') == 'configuracao') {
    			idRel = $('#configuracao .tab-pane:visible').data('idrel');
    			if (isNaN(idRel)) {
    				idRel = null;
    			}
    		}
    		$.fn.fullpage.moveSectionDown();
    		visent.agendamento.init(idRel);
    	});
    }
