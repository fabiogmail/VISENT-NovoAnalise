$(function execucao() {
	...
	function criarTabela(idExec, rel, params, resultados) {
		...
		for (var i=0; i < resultados.length; i++) {
			...
			var resumoHtml = '<table class="table table-striped table-bordered table-hover table-condensed">';
			var resumo = resultado.resumoExecucao;
			resumoHtml += '<tr><th class="text-center info" colspan="4">'+resumo.titulo+'</th></tr>';
			if (resultado.semDados.length > 0) {
				resumoHtml +='<tr class="danger"><th>Avisos</th><th colspan="3">';
				for (var j=0; j<resultado.semDados.length; j++) {
					resumoHtml += resultado.semDados[j] + '<br>';
				}
				resumoHtml +='</th></tr>';
			}
			resumoHtml += '<tr><td class="info">'+msgs['resumo.usuario']+'</td><td colspan="3">'+resumo.usuario+'</td></tr>';
			resumoHtml += '<tr><td class="info">'+msgs['resumo.relatorio']+'</td><td>'+resumo.nomeRelatorio+'</td><td class="info">'+msgs['resumo.tipo']+'</td><td>'+resumo.tipoRelatorio+'</td></tr>';
			resumoHtml += '<tr><td class="info">'+msgs['resumo.dataHoraExecucao']+'</td><td>'+resumo.dataHoraExecucao+'</td><td class="info">'+msgs['resumo.tempoExecucao']+'</td><td>'+resumo.tempoExecucao+'</td></tr>';
			resumoHtml += '<tr><td class="info">'+msgs['resumo.quantidadeLinhas']+'</td><td colspan="3">'+resumo.quantidadeLinhas+'</td></tr>';
			resumoHtml += '<tr><td class="info">'+msgs['resumo.data']+'</td><td>'+resumo.data+'</td><td class="info">'+msgs['resumo.periodos']+'</td><td>'+resumo.periodos+'</td></tr>';
			resumoHtml += '<tr><td class="info">'+msgs['resumo.colunas']+'</td><td colspan="3">'+resumo.colunas+'</td></tr>';
			resumoHtml += '<tr><td class="info">'+msgs['resumo.filtros']+'</td><td colspan="3">';
			for (var j=0; j<resumo.filtros.length; j++) {
				resumoHtml += resumo.filtros[j];
			}
			resumoHtml += '</td></tr>';
			resumoHtml += '</table>';
			$dlg.find('.execs-relatorios-titulos')
				.append('<li class="'+(i==0?'active':'')+'"><a id="tab-'+idCabecalho+'" href="#'+idCabecalho+'" data-toggle="tab">'+msgs['lbl.resumo']+'</a></li>');
			$dlg.find('.execs-relatorios-tabelas')
				.append('<div class="tab-pane '+(i==0?'active':'')+'" id="'+idCabecalho+'">'+resumoHtml+'</div>');
			break; // mudanca para exibir apenas 1 resumo
		}
		...
		for (var i=0; i < resultados.length; i++) {
			...
			for (var periodo in resultados[i].periodoLinhas) {
				...
				$dlg.find('.execs-relatorios-titulos')
					.append('<li><a id="tab-'+id+'" href="#'+id+'" data-toggle="tab">'+periodo+'</a></li>');
				$dlg.find('.execs-relatorios-tabelas')
					.append('<div class="tab-pane" id="'+id+'"><div class="table-responsive"><table class="table table-striped table-bordered table-hover table-condensed"></table></div></div>');
				j++;
