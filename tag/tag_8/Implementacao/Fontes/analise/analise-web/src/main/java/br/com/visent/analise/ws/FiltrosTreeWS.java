...
public class FiltrosTreeWS extends AbstractWS {
	...
	@POST
	@Path(Constants.Path.FILTROSTREE_COPY)
	@Consumes(MediaType.APPLICATION_JSON)
	@Produces(MediaType.APPLICATION_JSON)
	public void copiar(@PathParam("copyID") Long copyID, @PathParam("targetID") Long targetID) {
		ValoresTree copy = configuracaoService.getValoresTreeByID(copyID);
		ValoresTree target = dao.find(ValoresTree.class, targetID);
		ValoresTree pai = null;
		if (target.isNo()) {
			pai = target;
		} else {
			pai = target.getPai();
		...
		ValoresTree valoresTree = new ValoresTree();
		criarCopia(valoresTree, copy, pai);
		dao.persist(valoresTree);
	}

	private void criarCopia(ValoresTree valoresTree, ValoresTree copy, ValoresTree pai) {
		valoresTree.setNome(copy.getNome());
		valoresTree.setPai(pai);
		valoresTree.setNo(copy.isNo());
		if (valoresTree.isNo()) {
			valoresTree.setValoresFilhos(new ArrayList<>());
			List<ValoresTree> filhos = copy.getValoresFilhos();
			for (ValoresTree valoresTreeFilho : filhos) {
				ValoresTree filhoCopia = new ValoresTree();
				criarCopia(filhoCopia, valoresTreeFilho, valoresTree);
				valoresTree.getValoresFilhos().add(filhoCopia);
			}
		}
	}
